...
for y in year:
    ...
    for top in range(1,10):
        temp_profit_percent,temp_capital_compounding,temp_dates,final = process(file_path,max_percent,top)
        ...
        print(f,"Top: ",top,"Total capital at end:",capital_compounding[top])
        print(f,"Top: ",top,"Total profit percent:",total_profit[top])
        print(f,"Top: ",top,"Maximum Draw Down:",draw_down[top])
    fig = plt.figure()
    for top in range(1,10):
        plt.plot(dates[top],profit_percent_day[top],label = 'Top: ' + str(top))
    plt.xlabel('All dates in '+str(f))
    plt.ylabel('Total profit(starting from 0 at the start)')
    plt.legend()
    fig.savefig('Gap_Up_'+str(f)+'_different_top_total_profit_1.png',dpi=fig.dpi)
    ...
    plt.legend()
    fig.savefig('Gap_Up_'+str(f)+'_different_top_compounding_1.png',dpi=fig.dpi)
